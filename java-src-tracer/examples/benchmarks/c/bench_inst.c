...
# 4 "examples/bench.c" 2


# 5 "examples/bench.c"
int run(int n) { _FUNC(0x1)
    int sum = 0;
    { for (int i = 0; i < n; i++) { _LOOP_BODY(0x0)
        if ((i & 1) == 0) { _IF
            sum += i;
        } else { _ELSE
            sum -= i;
...
int main(int argc, char *argv[]) { _TRACE_OPEN("/mnt/c/Users/salma/java-src-tracer/examples/c-bench/%F-%H%M%S-%%lx-bench.c")  _FUNC(0x2)
    int n = argc > 1 ? atoi(argv[1]) : 1000000;
    run(10000);

    struct timespec start, end;
    clock_gettime(
# 22 "examples/bench.c" 3 4
                 1
# 22 "examples/bench.c"
                                , &start);
    int result = run(n);
    clock_gettime(
# 24 "examples/bench.c" 3 4
                 1
# 24 "examples/bench.c"
                                , &end);
